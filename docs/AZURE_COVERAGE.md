...
Regenerate with `pnpm tsx scripts/azure-coverage-benchmark.mts`; the
canonical machine-readable copy lives at `scripts/output/azure-coverage.{md,json}`.

## How to read this

- **Recognised by PDP** — the action is registered in
  `@auto-nomos/schema-packs` and the PDP boot-loaded it. A non-recognised
  entry would deny with `unknown_command` and is treated as a build/deploy
  drift failure.
- **UCAN minted** — `/v1/mint-ucan` accepted `cloudConnectionId` and bound a
  UCAN with `meta.cloud_connection_id`. The 1 miss is `/azure/raw_call`
  (escape hatch — separate mint path).
- **Broker forwarded to ARM** — Cedar policy allowed, the cosigner gate did
  not fire, and PDP successfully exchanged the OIDC ID token with Entra ID
  and made the ARM call. The HTTP status the customer sees from upstream
  is included so you can tell broker bugs (502/missing fields) from real
  upstream rejections (403 RBAC, 404 not found).
- **Cosigner-gated** — destructive verb hit the
  `apps/pdp/src/services/cloud-risk-rules.ts` allowlist (delete, purge,
  rotate, drain, scale, stop, terminate, regenerate_key, deallocate,
  reimage, run_command, invoke, redeploy, slot_swap, remove_rule,
  detach_disk, capture, uninstall_extension, cancel, cancel_run,
  power_off). Returns 403 `cosigner_required` regardless of policy.
- **Schema violation** — PDP rejected the apiCall shape before Cedar
  (wrong method/missing api-version). Zero is the expected value.

## Phase B coverage (not yet exercised)

The current benchmark runs with the prod Azure connection bound to a
**Reader-scoped role assignment**. That's why writes/destructives end at
the cosigner gate (the gate fires before reaching ARM, so we couldn't
mutate anyway). For full end-to-end mutation testing:

1. Provision a sandbox resource group with `Contributor` scoped only to
   that RG.
2. Add an alternate Nomos cloud connection that points at the same
   subscription but a different App Registration with Contributor.
3. Re-run the benchmark with `NOMOS_CLOUD_CONN_ID` pointed at the
   sandbox connection.
4. Each destructive case becomes a real create→delete cycle with the
   broker holding the cosigner gate open via session-scoped approval —
   then `agents.update` revokes the agent and the harness self-deletes
   created resources through `/v1/proxy`. Tracked as task #19 in this
   session.
